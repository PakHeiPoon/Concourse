...
| `concourse.paking.xyz` 域名 + Vercel 部署 | **保留**。**indexer + 签名仪式页同样的托管故事**。 |
| `api.concourse.paking.xyz` FastAPI 后端 | **保留**（瘦身）。Auth + drafts + indexer cache + 多租户 runtime。 |
...
```
旧：api.concourse.paking.xyz/mcp/tools/execute
